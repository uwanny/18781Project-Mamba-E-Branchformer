...
class EBranchformerEncoderLayerMambaParallel(torch.nn.Module):
    """E-Branchformer encoder layer module parallel with mamba.
    ...
    def __init__(
        ...
        self.norm_mamba = LayerNorm(size)  # for the mamba module
        ...
        self.depthwise_conv_fusion = torch.nn.Conv1d(
            ...
            groups=size + size + size,
            bias=True,
        )
        self.merge_proj = torch.nn.Linear(size + size + size, size)
...
class EBranchformerEncoderMambaParallel(AbsEncoder):
    """E-Branchformer encoder module merged with parallel mamba."""
    ...
    def __init__(
        ...
        if Mamba_type == "Bimamba_outer":
            ...
            raise ValueError(
                f"unknown mamba type: {Mamba_type}, only supports Bimamba_outer"
            )

        self.encoders = repeat(
            num_blocks,
            lambda lnum: EBranchformerEncoderLayerMambaParallel(
                output_size,
                mamba_layer(*mamba_layer_args),
